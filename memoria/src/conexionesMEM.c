        ...
        switch(paquete->codigo_operacion) {
            ...
            case ESCRIBIR_STDOUT:
                ...
                if(result_io == 0) {
                    ...
                    printf("el valor del registro lectura es :%s\n", (char*)registro_lectura);
            ...
            case ESCRIBIR_FS_MEMORIA: // Lee en memoria y envia a fs
                ...
                // EL PRINT DE ANTES ERA
                printf("El registro leido es: %s\n", (char*)registro_lectura_rw);
...
void* handle_io_dialfs(void* socket) {
    ...
    printf("Se conecto una io dialfs!\n");
    ...
    while(1) {
        ...
        // Aca voy a validar la conexion si el socket devolvio -1,
        if(paquete->codigo_operacion == -1) {
            printf("Se desconecto el IO\n");
            desconectar_io_de_diccionario(paquete_inicial->buffer->stream);

            // Liberar todo lo que sea necesario  del lado de memoria
        ...
        switch(paquete->codigo_operacion) { 
            case ESCRIBIR_MEM_FS:  
                ...
                imprimir_datos_escritura_fs(escritura_lectura);

                void* registro_escritura = escritura_lectura->valor_a_copiar;
                ...
                send(socket_io, &confirm_escritura_fs, sizeof(uint32_t), 0);
                break;
            default:
                printf("Rompio todo?\n");
                return NULL;
...
void imprimir_datos_escritura_fs(t_escritura_memoria_fs* escritura_lectura) {
    printf("El registro tamanio es: %d\n", escritura_lectura->registro_tamanio);
    printf("La cantidad de paginas es: %d\n", escritura_lectura->cantidad_paginas);
    printf("El pid es: %d\n", escritura_lectura->pid);
    printf("El tamanio a copiar es: %d\n", escritura_lectura->tamanio_a_copiar);
    printf("El valor a copiar es: %s\n", (char*)escritura_lectura->valor_a_copiar);

    for(int i = 0; i < escritura_lectura->cantidad_paginas; i++) {
        t_dir_fisica_tamanio* dir_tam = list_get(escritura_lectura->lista_dir_tamanio, i);
        printf("Direccion fisica: %d\n", dir_tam->direccion_fisica);
        printf("Bytes de lectura: %d\n", dir_tam->bytes_lectura);
    }
}
...
void mandar_lectura_a_fs(int pid, void* registro_lectura, t_pedido_rw_encolar* pedido_fs, uint32_t size_registro, int socket_io) {
    ...
    buffer->size = sizeof(int) * 3 + size_registro + sizeof(uint32_t) + pedido_fs->largo_archivo;
    ...
    memcpy(stream + buffer->offset, &pid, sizeof(int));
    buffer->offset += sizeof(int);
    memcpy(stream + buffer->offset, &size_registro, sizeof(uint32_t));
    buffer->offset += sizeof(uint32_t);
...
t_escritura_memoria_fs* deserializar_escritura_lectura_fs(t_buffer* buffer) {
    t_escritura_memoria_fs* escritura_fs = malloc(sizeof(t_escritura_memoria_fs));

    // Hacer la funcion en base al comentario de arriba con los campos que tiene que tener

    void* stream = buffer->stream;

    memcpy(&escritura_fs->pid, stream, sizeof(int));
    stream += sizeof(int);
    memcpy(&escritura_fs->registro_tamanio, stream, sizeof(uint32_t));
    stream += sizeof(uint32_t);
    memcpy(&escritura_fs->cantidad_paginas, stream, sizeof(int));
    stream += sizeof(int);
    memcpy(&escritura_fs->tamanio_a_copiar, stream, sizeof(int));
    stream += sizeof(int);

    escritura_fs->valor_a_copiar = malloc(escritura_fs->tamanio_a_copiar);
    memcpy(escritura_fs->valor_a_copiar, stream, escritura_fs->tamanio_a_copiar);
    stream += escritura_fs->tamanio_a_copiar;

    escritura_fs->lista_dir_tamanio = list_create();

    for(int i = 0; i < escritura_fs->cantidad_paginas; i++) {
        t_dir_fisica_tamanio* dir_tam = malloc(sizeof(t_dir_fisica_tamanio));
        memcpy(&dir_tam->direccion_fisica, stream, sizeof(int));
        stream += sizeof(int);
        memcpy(&dir_tam->bytes_lectura, stream, sizeof(int));
        stream += sizeof(int);

        list_add(escritura_fs->lista_dir_tamanio, dir_tam);
    }
...
void liberar_modulo_memoria() { //NO SE USA
...
void eliminar_instrucciones(int pid) { //NO SE USA
    /*
...
void _liberar_pagina(t_pagina* pagina) { //NO SE USA
    free(pagina);
}

void liberar_tablas_paginas() { //NO SE USA
    dictionary_destroy_and_destroy_elements(diccionario_tablas_paginas, (void*)destruir_paginas);
}

void liberar_ios() { //NO SE USA
...
void mostrar_en_io(int socket_io, char* valor_leido) { //NO SE USA
